jQuery(document).ready(function ($) {
    jQuery('#lichAmDuong').submit(function (e) {
        ...
        jQuery.ajax({
            ...
            dataType: "JSON",
            beforeSend: function () {
                $(this).find('.jeg_readmore').text('Loading...');
                $(this).find('.jeg_readmore').prop('disable', true);
            },
                ...
                if (response.success === true) {
                    ...
                    switch (response.data.type) {
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            $('#lad-text-1').html('Âm lịch lịch - dương lịch ngày ' +lunar_day+ '/' +lunar_month+ '/' +lunar_year);
                            break;
                    }
                } else {
                    alert('Có lỗi xảy ra !!');
                }
                $(this).find('.jeg_readmore').text('Xem kết quả');
                $(this).find('.jeg_readmore').prop('disable', false);
            }
        })
    });
});